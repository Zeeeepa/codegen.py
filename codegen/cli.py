...
from .core import CodegenClient, ConfigPresets
...
class CodegenCLI:
    ...
    def get_agent(self) -> Agent:
        ...
        if not self.agent:
            self.agent = Agent(org_id=self.config.get("org_id"), token=self.config.get("token"))
        return self.agent
...
Examples:
  ...
  codegen logs 123 --follow
    ...
    run_parser.add_argument(
        "--poll-interval", type=float, help="Polling interval in seconds (default: 5.0)"
    )
    ...
    status_parser.add_argument(
        "--log-limit", type=int, help="Number of log entries to show (default: 10)"
    )

    # Config command
    config_parser = subparsers.add_parser("config", help="Manage configuration")
    config_parser.add_argument(
        "action", choices=["show", "set", "preset", "init"], help="Configuration action"
    )
    ...
    monitor_parser.add_argument(
        "--limit", type=int, help="Number of tasks to monitor (default: 20)"
    )
    monitor_parser.add_argument(
        "--interval", type=int, help="Monitoring interval in seconds (default: 10)"
    )
