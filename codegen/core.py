...
class CodegenClient:
    ...
    def __init__(self, org_id: Optional[str] = None, token: Optional[str] = None, config: Optional[ClientConfig] = None):
        ...
        if not self.token:
            raise ValueError("API token is required. Set CODEGEN_API_TOKEN environment variable or pass token parameter.")
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    raise AuthenticationError("Invalid API token", response.status_code, response.json())
                    ...
                    raise ServerError(f"Server error: {response.status_code}", response.status_code, response.json())
                else:
                    raise CodegenAPIError(f"HTTP {response.status_code}", response.status_code, response.json())
                    
            except (requests_exceptions.Timeout, requests_exceptions.ConnectionError) as e:
                if attempt == self.config.max_retries:
                    raise NetworkError(f"Network error after {self.config.max_retries + 1} attempts: {str(e)}")
    ...
    def run_agent(self, prompt: str, **kwargs) -> AgentRunResponse:
        ...
        response = self._make_request("POST", f"/v1/organizations/{self.org_id}/agent/run", json=data)
    ...
    def get_agent_run_logs(self, run_id: int, skip: int = 0, limit: int = 100) -> AgentRunWithLogsResponse:
        """Get logs for a specific agent run"""
        params = {"skip": skip, "limit": limit}
        response = self._make_request("GET", f"/v1/organizations/{self.org_id}/agent/run/{run_id}/logs", params=params)
    ...
    def list_agent_runs(self, skip: int = 0, limit: int = 100) -> AgentRunsResponse:
        ...
        params = {"skip": skip, "limit": limit}
        response = self._make_request("GET", f"/v1/organizations/{self.org_id}/agent/runs", params=params)
