"""
Starred runs frame for the Codegen UI.

This module provides a frame for displaying starred agent runs.
"""
...
class StarredRunsFrame(ttk.Frame):
    """
    Frame for displaying starred agent runs.
    
    This frame displays a list of starred agent runs with filtering and pagination.
    """
    
    def __init__(self, parent, controller):
        """
        Initialize the starred runs frame.
    ...
    def _create_widgets(self):
        """Create widgets for the starred runs frame."""
        # Create header frame
    ...
    def load_starred_runs(self):
        ...
        try:
            ...
            # Get starred runs
            starred_run_ids = self.controller.get_starred_agent_runs()
            
            # Get agent runs
            all_runs = self.controller.get_agent_runs()
            
            # Filter starred runs
            self.agent_runs = [run for run in all_runs if run.get("id") in starred_run_ids]
            
            # Update agent run cards
            self._update_agent_run_cards()
            
        except Exception as e:
            logger.error(f"Error loading starred runs: {e}")
            ...
            error_label = ttk.Label(
                self.scrollable_frame,
                text=f"Error loading starred runs: {str(e)}",
                style="Error.TLabel"
            )
            error_label.pack(pady=PADDING * 2)
    
    def show(self):
        """Show the starred runs frame."""
        self.pack(fill=tk.BOTH, expand=True)
        self.load_starred_runs()
    
    def hide(self):
        """Hide the starred runs frame."""
        self.pack_forget()
